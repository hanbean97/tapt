...
public class previewbluck : MonoBehaviour
{
    ...
    public void SpawnPiece()//새로운 블록 정보 갱신해야할때 호출
    {
        ...
        blocktable.RemoveAt(randomindex);//선택된 블록은 앞으로 나올 블록에서 제거
        Databiuck data = datablock[random];//해당 번호에 해당하는 블록 모양 데이터

        activepiece.Initialize(data);//나올 블록의 방향과 모양 데이터에 해당 데이터를 넣어준다

        blockview();//인게임상 보여주는 블록 활성화
    }
